...
The purpose of `ty` is not to be used as a CLI application or just as a Rust library, but to be ran on any platform, focused primarily on the client. This partly explains why its so minimal. It can be used in web-based projects through WebAssembly. The library also ensures usability in other languages, like Go or Swift, with its FFI bindings.
